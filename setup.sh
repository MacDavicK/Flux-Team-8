...
check_ngrok_container_running() {
    [[ -n "$(docker compose --project-directory "$BACKEND_DIR" --profile ngrok ps ngrok -q 2>/dev/null)" ]]
}
...
run_docker() {
    ...
    if $use_ngrok; then
        sleep 5
        if docker compose --project-directory "$BACKEND_DIR" logs ngrok 2>&1 | grep -q "ERR_NGROK_334\|endpoint already online"; then
            warn "ngrok endpoint held elsewhere — Twilio webhooks may not work."
            info "Stop the tunnel at dashboard.ngrok.com/endpoints, then restart."
        fi
    fi
    ...
    elif [[ -n "${SKIP_NGROK:-}" ]]; then
        info "ngrok:     skipped (SKIP_NGROK=1)"
    fi
    echo
