...
uvicorn scrum_40_notification_priority_model.main:app --host 0.0.0.0 --port 8040 --reload
```
...
| Interface | URL |
|---|---|
| **Swagger UI** (interactive) | `http://localhost:8040/docs` |
| **ReDoc** (read-only) | `http://localhost:8040/redoc` |
| **OpenAPI JSON spec** | `http://localhost:8040/openapi.json` |

Once running, open `http://localhost:8040/docs` in your browser to explore and test all endpoints interactively.
...
All `Notification Priority Model API` endpoints will be available at `http://localhost:8000/docs` alongside the other modules.
