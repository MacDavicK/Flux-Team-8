...
if [ $UNIT_EXIT -eq 0 ]; then
    ...
    docker compose -f "$PROJECT_ROOT/docker-compose.yml" down dao 2>/dev/null || true

    if docker compose -f "$PROJECT_ROOT/docker-compose.yml" up -d dao 2>&1; then
        success "Container deployed: dao"
        ...
        error "Try: docker compose logs dao"
...
if [ "$CONTAINER_DEPLOYED" = true ]; then
    ...
    if [ "$HEALTH_OK" = true ]; then
        success "Service is healthy: http://localhost:8001/health"
...
- Root \`docker-compose.yml\` — Service deployment (dao service)
...
if [ "$CLEANUP" = true ] && [ "$CONTAINER_DEPLOYED" = true ]; then
    ...
    docker compose -f "$PROJECT_ROOT/docker-compose.yml" down dao 2>/dev/null
...
if [ "$CONTAINER_DEPLOYED" = true ] && [ "$CLEANUP" = false ]; then
    info "DAO service is running at http://localhost:8001"
    info "Swagger UI: http://localhost:8001/docs"
    info "To stop: docker compose -f docker-compose.yml down dao"
fi
