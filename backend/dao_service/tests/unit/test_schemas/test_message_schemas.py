...
class TestMessageCreateDTO:
    def test_valid_message_minimal(self):
        dto = MessageCreateDTO(conversation_id=uuid4(), role="user", content="Hello")
    ...
    def test_invalid_role_raises_error(self):
        with pytest.raises(ValidationError):
            MessageCreateDTO(conversation_id=uuid4(), role="admin", content="Hello")
    ...
    def test_empty_content_raises_error(self):
        with pytest.raises(ValidationError):
            MessageCreateDTO(conversation_id=uuid4(), role="user", content="")

    def test_all_valid_roles(self):
        for role in ("user", "assistant", "system", "function"):
            dto = MessageCreateDTO(conversation_id=uuid4(), role=role, content="test")
