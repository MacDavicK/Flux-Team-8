...
from app.services.user_notes import get_user_notes
...
async def pattern_observer_node(state: AgentState) -> dict:
    ...
    # Load stored user preference notes so the LLM can cross-reference known habits
...
async def flag_goal_milestone_completion(
    user_id: str, goal_id: str, milestone_title: str, pipeline_order: int
) -> None:
    """
    Records a milestone_completion pattern when a pipeline-goal milestone is
    completed and the user advances to the next one.

    This signals to the pattern_observer that the time slots previously occupied
    by this milestone's tasks are now free again — so the scheduler won't treat
    them as conflicts when planning the next milestone.
    """
    pattern_key = f"milestone_completed_{goal_id}_{pipeline_order}"

    existing = await db.fetchrow(
        """
        SELECT id FROM patterns
        WHERE user_id = $1 AND pattern_type = 'milestone_completion' AND pattern_key = $2
        """,
        user_id,
        pattern_key,
    )
    ...
    completed_count = await db.fetchval(
        "SELECT COUNT(*) FROM tasks WHERE goal_id = $1 AND user_id = $2 AND status = 'done'",
        goal_id,
        user_id,
    )
    total_count = await db.fetchval(
        "SELECT COUNT(*) FROM tasks WHERE goal_id = $1 AND user_id = $2 AND status != 'cancelled'",
        goal_id,
        user_id,
    )
    done: int = completed_count or 0
    total: int = total_count or 1
    completion_rate: float = done / max(total, 1)
    confidence: float = min(completion_rate, 1.0)

    await db.execute(
        """
        INSERT INTO patterns (user_id, pattern_type, pattern_key, description, confidence, data)
        VALUES ($1, 'milestone_completion', $2, $3, $4, $5::jsonb)
        """,
        user_id,
        pattern_key,
