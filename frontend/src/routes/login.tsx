...
function LoginPage() {
  ...
  const postLogin = (resolvedUser: { onboarded?: boolean } | undefined) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        const resolvedUser = await signup(name.trim(), email, password);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Browser navigates away — no further state update needed here.
    } catch (err) {
      setGoogleError(
          ...
          <div className="mt-6 pt-4 border-t border-glass-border">
            ...
            {googleError && (
              <motion.div
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-terracotta/10 border border-terracotta/30 rounded-bubble p-3 mb-3"
              >
