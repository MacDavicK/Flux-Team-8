...
async function handleGoalPlannerMessage(request: Request) {
  ...
  switch (state) {
    ...
    case AgentState.GATHERING_TIMELINE:
      ...
      newState = AgentState.GATHERING_CURRENT_STATUS;
      response = {
        message:
    ...
    case AgentState.GATHERING_CURRENT_STATUS:
      newContext.currentStatus = message;
      newState = AgentState.GATHERING_TARGET_STATUS;
      response = {
    ...
    case AgentState.GATHERING_TARGET_STATUS:
      if (input.includes("suggest")) {
        newContext.targetStatus = "75 kg";
      } else {
        newContext.targetStatus = message;
      }
