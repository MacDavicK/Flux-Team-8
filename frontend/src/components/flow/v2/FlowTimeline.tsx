...
interface FlowTimelineProps {
  events: Event[];
  onShuffle?: (eventId: string, taskTitle: string) => void;
}

export function FlowTimeline({ events, onShuffle }: FlowTimelineProps) {
  return (
        ...
        {events.map((event, index) => (
          <div key={event.id} className={event.isPast ? "opacity-70" : ""}>
            <TimelineEvent
              ...
              isDrifted={event.state === "drifted"}
              onShuffle={
                onShuffle ? () => onShuffle(event.id, event.title) : undefined
              }
            />
